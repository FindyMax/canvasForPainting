...
public class AlbumView extends View {
    ...
    private Canvas canvasCheck; // вторичный холст для предварительного просмотра
    ...
    private Bitmap bitmapCheck; // контейнер, где будет храниться вторичный рисунок
    private final int PAINT_COLOR_DEF = 0xFF4C9063; // цвет по умолчанию

    // координаты касания View
    private float xStart = 0, yStart = 0, xMove = 0, yMove = 0, xFinish = 0, yFinish = 0, distance = 0;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        // создание пустого изображения с заданной шириной, высотой и цветовой схемой (альфа, красный, зеленый, голубой)
        bitmapCheck = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888);
        // создание холста для созданного изображения
        canvas = new Canvas(bitmap);
        // создание вторичного холста для создаваемого изображения
        canvasCheck = new Canvas(bitmapCheck);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawBitmap(bitmapCheck, 0, 0, paint); // помещение на холст отрисованных линий
        ...
        if (figure.equals("drawing")) {
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN: // прикосновение к View
                    ...
                    break;
                case MotionEvent.ACTION_MOVE: // движение по экрану
                    // очистка вторичного холста
                    checkClear();
                    // извлечение координат касания View
                    xMove = event.getX();
                    yMove = event.getY();
                    distance = (float) Math.sqrt((xMove - xStart)*(xMove - xStart) + (yMove - yStart)*(yMove - yStart));
                    // отрисовка предварительной окружности
                    canvasCheck.drawCircle(xStart,yStart,distance,paint);
                    break;
                case MotionEvent.ACTION_UP: // отпускание экрана
                    // очистка вторичного холста
                    checkClear();
                    ...
                    distance = (float) Math.sqrt((xFinish - xStart)*(xFinish - xStart) + (yFinish - yStart)*(yFinish - yStart));
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE: // движение по экрану
                    ...
                    xMove = event.getX();
                    yMove = event.getY();
                    // отрисовка предварительной линии
                    canvasCheck.drawLine(xStart,yStart,xMove,yMove,paint);
                    break;
                case MotionEvent.ACTION_UP: // отпускание экрана
                    // очистка вторичного холста
                    checkClear();
                    ...
                    xFinish = event.getX();
                    yFinish = event.getY();
                    // отрисовка линии (координаты первой точки, координаты второй точки, стиль рисования)
    ...
    // метод очистки вторичного холста
    public void checkClear() {
        // очистка экрана и заполнение его белым цветом
        canvasCheck.drawColor(0xFFFFFFFF, PorterDuff.Mode.CLEAR);
        // обновление представления View
        invalidate();
    }
}
